...
var filterHiddenData = function filterHiddenData(list) {
  if (!isArray(list)) {
    return list;
  }
  var result = [];
  for (var i = 0; i < list.length; i++) {
    ...
    if (!item.hidden) {
      ...
      if (item.children) {
        var children = filterHiddenData(item.children);
        if (children.length > 0) {
          node.children = children;
        }
      }
      if (item.items) {
        var items = filterHiddenData(item.items);
        if (items.length > 0) {
          node.items = items;
        }
      }
...
export var getExtraView = function getExtraView(extraKey, schema, widgets) {
...
/*
   ...
   */
export var getDependValues = function getDependValues(formData, dependPath, props, dependencieItem) {
  var indexReg = /\[[0-9]*\]/;
